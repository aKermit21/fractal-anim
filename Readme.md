...
(see [external documentation](https://fractal.pcc21.com/) for more detailed info).
...
- tomlplusplus (toml config parser) - when using Meson build system shall be automatically fetched from web and build as subproject at first build setup call.
## Installation
Clone GitHub project
### Using Meson
It is recommended to use Meson build system as it handles dependencies including subprojects,
...
but this may require some extra manual tweaks.
One needs for sure perform manually additional two/three steps:
- edit config.h.in manually in src/ and change file name to config.h;
- install tomlplusplus (toml++) shared library in system;
- add aditional includes if needed in Makefile.
Then:
``` shell
cd fractal-anim
...
To modify and test changed code it is recommended to
...
```
Build type diffrent than release will switch on asserts and extensive logging.

Alternatively one can use 
`make all`.

## External website
For more technical info please visit page [Documentation](https://fractal.pcc21.com/)
